%% Analysis Data
[Actual,fs] = audioread('wooden-impact.wav'); % the audio data
[allMag, allFreq] = readSpearFile('wooden-impact.txt'); % sinusoidal analysis data from Spear
hopSize     = fs / 100; % Spear exports data with 10ms frames
saveResults = 1;    % save the results to a mat file?
matNameMag  = 'resultsWoodenImpact_3.mat'; % name of the .mat file saved if saveResults = 1
...
% first we must select our modes of vibration, and format the data
 ...
 param_guess = 1; % make an inital parameter guess? 1 = use D_ and S_, 0 = try random
    ...
    D_ = 0.2*rand(numModes,1) + 5; % guess for damping (partly randomised)
    S_ = 0.2*rand(numModes,1) + 0.5; % guess for stiffness (partly randomised)
