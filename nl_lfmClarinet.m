%% Analysis Data
[Actual,fs] = audioread('clarinetC4.wav'); % the audio data
[allMag, allFreq] = readSpearFile('clarinetC4.txt'); % sinusoidal analysis data from Spear
hopSize     = fs / 100; % Spear exports data with 10ms frames
saveResults = 1;    % save the results to a mat file?
matNameMag  = 'resultsClarinet_2.mat'; % name of the .mat file saved if saveResults = 1
...
% first we must select our modes of vibration, and format the data
 ...
 polyOrder = 0; % I already removed most partials from the spear file manually so this can be 0
 ...
 param_guess = 1; % make an inital parameter guess? 1 = use D_ and S_, 0 = try random
    % if you change gamma, you should also make a different guess
    D_ = 0.2*rand(numModes,1) + 0.5; % guess for damping (partly randomised)
    S_ = 0.2*rand(numModes,1) + 0.3; % guess for stiffness (partly randomised)
...
if priorityInference == 1
    n_sigma2 = 0.00001 ./ maxOfMode.^(5/3) % priority inference. WJW - Feb 2017
else
...
%Bj = .1*rand(d,1);
Bj = zeros(d,1);
Dj = 0.2*rand(d,1) + 0.5; % overwritten if param_guess = 1
Sj = 0.2*rand(d,1) + 0.3; % overwritten if param_guess = 1
